...
const root = {
    ...
    addNewEmp: async ({ first_name, last_name, email, gender, designation, salary, 
      date_of_joining, department, other, employee_photo}, req) => {
        const user = authenticate(req);
        if (!user) {
          throw new Error("Unauthorized access");
        }
        try{
          const newEmp = new Emp({ first_name, last_name, email, gender, designation, salary, 
            date_of_joining, department, other, employee_photo })
            await newEmp.save()
            return newEmp
        }catch (error) {
    ...
    updateEmpById: async ({ eid, first_name, last_name, email, gender, designation, salary, 
      ...
      try {
        const updatedEmp = await Emp.findByIdAndUpdate(
          eid,
          { first_name, last_name, email, gender, designation, salary, date_of_joining, department, other, employee_photo },
          { new: true }
        );
        if (!updatedEmp) {
          throw new Error("Employee not found");
